...
model_path = 'global_federated_rf_model.pkl'
try:
    global_rf = joblib.load(model_path)
    print(f"Successfully loaded Global RF Model with {global_rf.n_estimators} trees.")
except Exception as e:
    print(f"Error: Could not load the model. Did you run the server script? {e}")
    exit()
...
print(f"Unique labels in test data: {unique_classes}")
...
y_probs = global_rf.predict_proba(X_test_scaled)[:, 1]

# Find the threshold that balances precision and recall best
precision, recall, thresholds = precision_recall_curve(y_test, y_probs)
# Instead of maximizing F1, find the threshold where Precision is at least 0.50
target_precision = 0.50
idx = np.where(precision >= target_precision)[0]

if len(idx) and idx[0] < len(thresholds) > 0:
    best_threshold = thresholds[idx[0]]
else:
    # Fallback to a much higher fixed threshold if target can't be met
...
print(f"Applying Precision-Focused Threshold: {best_threshold:.4f}")
